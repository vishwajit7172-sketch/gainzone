...
export default function ExerciseLibrary() {
  ...
  useEffect(() => {
    if (selected) {
      // Push a dummy history entry so back button hits this first
  ...
  // Uses position:fixed trick which works on iOS Safari
  useEffect(() => {
    if (!selected) return
  ...
  return (
    <div className="slide-up">
      ...
      {/* ── MODAL ── renders as a full-screen overlay, scrolls internally */}
      {selected && (
        <div
          ...
          style={{
            ...
            zIndex: 9999,
            // The backdrop itself scrolls — modal content rides inside it
            overflowY: 'scroll',
            WebkitOverflowScrolling: 'touch' as any,
          }}
        >
          {/* Close tap area at top on mobile */}
          <div onClick={closeModal} style={{ height: '20px', width: '100%' }} />
          ...
          >
            ...
            <div style={{ background: 'var(--surface2)', padding: '1.25rem 1.5rem 1rem', borderRadius: '12px 12px 0 0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                ...
                <button
                  onClick={closeModal}
                  style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--muted)', width: '36px', height: '36px', borderRadius: '6px', cursor: 'pointer', fontSize: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                >×</button>
              </div>
