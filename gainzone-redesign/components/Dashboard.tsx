      ...
      {isRest ? (
        ...
        <div className="today-banner" style={{ marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => onNavigate('exercises')} className="btn-primary">View ↗</button>
          </div>
        </div>
      )}
            ...
            {todayExes.slice(0, 4).map(ex => ex && (
              <div key={ex.id} className="card-glow" style={{ cursor: 'pointer', overflow: 'hidden', padding: 0 }} onClick={() => onNavigate('exercises')}>
                ...
                <div style={{ padding: '10px 12px' }}>
                  ...
                  <div style={{ fontSize: '12px', fontWeight: 500, marginBottom: '2px', lineHeight: 1.3 }}>{ex.name}</div>
